...
class DinnerModal extends React.Component {
  ...
  render() {
    ...
    for(let i = 0; i < this.props.dinnerArray.length; i++){
        ...
        foodRows.push(<Text style={styles.mealHeader}>{this.props.dinnerArray[i].name}{"\n"}</Text>);
        foodRows.push(<Text style={styles.mealInformation}>Calories: {this.props.dinnerArray[i].calories}{"\n"}</Text>);
        foodRows.push(<Text style={styles.mealInformation}>Carbohydrates: {this.props.dinnerArray[i].carbohydrates} (g){"\n"}</Text>);
        foodRows.push(<Text style={styles.mealInformation}>Fat: {this.props.dinnerArray[i].fat} (g){"\n"}</Text>);
        foodRows.push(<Text style={styles.mealInformation}>Protein: {this.props.dinnerArray[i].protein} (g){"\n"}</Text>);
        foodRows.push(<Text>{"\n"}</Text>);
    }

    if(this.props.dinnerArray.length != 0){ // don't want to list the total if nothing is there
        totalRows.push(<Text style={styles.mealHeader}>Dinner Totals:{"\n"}</Text>);
    ...
    if(this.props.dinnerArray.length == 0){ // base case -- no activities added to user
      ...
      return (
        <View style={{position: 'absolute'}}>
          ...
          <View style={{position: 'absolute', width: this.props.width, height: this.props.height, left: (screenWidth - this.props.width)/2, top: (screenHeight - this.props.height)/2, backgroundColor: 'white', borderRadius: 10, marginTop: -180}}>
            <Text style={styles.instructions}>Dinner</Text>
            <Text>{foodRows}</Text>
                <Text>{totalRows}</Text>
