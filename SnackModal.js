...
class SnackModal extends React.Component {
  ...
  render() {
    ...
    for(let i = 0; i < this.props.snackArray.length; i++){
        ...
        foodRows.push(<Text style={styles.mealHeader}>{this.props.snackArray[i].name}{"\n"}</Text>);
        foodRows.push(<Text style={styles.mealInformation}>Calories: {this.props.snackArray[i].calories}{"\n"}</Text>);
        foodRows.push(<Text style={styles.mealInformation}>Carbohydrates: {this.props.snackArray[i].carbohydrates} (g){"\n"}</Text>);
        foodRows.push(<Text style={styles.mealInformation}>Fat: {this.props.snackArray[i].fat} (g){"\n"}</Text>);
        foodRows.push(<Text style={styles.mealInformation}>Protein: {this.props.snackArray[i].protein} (g){"\n"}</Text>);
        foodRows.push(<Text>{"\n"}</Text>);
    }

    if(this.props.snackArray.length != 0){ // don't want to list the total if nothing is there
        totalRows.push(<Text style={styles.mealHeader}>Snack Totals:{"\n"}</Text>);
    ...
    if(this.props.snackArray.length == 0){ // base case -- no activities added to user
      ...
      return (
        <View style={{position: 'absolute'}}>
          ...
          <View style={{position: 'absolute', width: this.props.width, height: this.props.height, left: (screenWidth - this.props.width)/2, top: (screenHeight - this.props.height)/2, backgroundColor: 'white', borderRadius: 10, marginTop: -180}}>
            <Text style={styles.instructions}>Snack</Text>
            <Text>{foodRows}</Text>
